...
public class GetBootstrapUserQueryHandler(IUserService userService, IRequestInterpretorService requestInterpretorService, ISystemContextFactory systemContextFactory) : IRequestHandler<GetBootstrapUserQuery, GetBootstrapUserResponse>
{
    public async Task<GetBootstrapUserResponse> Handle(GetBootstrapUserQuery request, CancellationToken cancellationToken)
    {
        if (!requestInterpretorService.IsAuthenticated())
        {
