...
public class CreateUserCommandHandler(IUserService userService, IMediator mediator) : IRequestHandler<CreateUserCommand, CreateUserResponse>
{
    ...
    {
        var response = await userService.CreateUser(new(request.CreateUserRequest.Email, request.CreateUserRequest.PhoneNumber, request.CreateUserRequest.IsActive, false, request.CreateUserRequest.Password, request.CreateUserRequest.DisplayName, request.CreateUserRequest.AssignedRoles), cancellationToken).ConfigureAwait(false);
