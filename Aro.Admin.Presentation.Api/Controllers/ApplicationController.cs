...
#pragma warning restore CS9113 // Parameter is unread.
{
    ...
    //    logger.LogDebug("Starting InitializeSystem operation for email: {Email}, displayName: {DisplayName}", model.Email, model.DisplayName);
        ...
        var response = await mediator.Send(new InitializeSystemCommand(new(model.Email, model.PhoneNumber, model.Password, model.DisplayName, model.BootstrapPassword)), cancellationToken).ConfigureAwait(false);
